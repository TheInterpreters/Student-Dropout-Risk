...
replace_text(
    scope_paragraph,
    "All predictive modelling and explanation evaluation will use one openly licensed UCI dataset. Short, consented written responses from the human evaluation will form the only unstructured component and will remain linked to the study cases. No external review corpus or second modelling dataset will be included, keeping the prediction task, explanations and human evidence coherently connected.",
)
...
replace_text(
    ...
    "The primary prediction task will be binary: Dropout (1) versus Graduate (0). Enrolled records will be excluded as unresolved outcomes rather than confirmed non-dropout cases; this will simplify interpretation but may introduce selection bias and limit applicability to similar unresolved students. A meaningful share of Dropout records (17.3%, versus 3.4% of Graduate records) show no first-semester evaluations, so some labelled dropouts may have left before the decision point rather than after it; this timing ambiguity will be reported as an explicit eligibility limitation. The resulting class distribution will be documented. Subject to time and class support, sensitivity analysis will retain Enrolled as a separate class or group it with non-dropout, while primary conclusions will stay tied to the pre-specified binary task.",
)

replace_text(
    find_one(document, "The prediction time is immediately after first-semester results"),
    "The prediction time will be immediately after first-semester results. Only variables available by then will enter the model: enrolment/admission records, demographic and socio-economic fields, administrative status, macroeconomic indicators, and first-semester outcomes. Second-semester curricular variables will be excluded as temporal leakage. First-semester grades and approved units are predictive but already historical, so they will be treated as immutable in counterfactual recourse.",
)
...
replace_text(
    ...
    "As a TFM is not a tree ensemble, TreeSHAP exactness will not be claimed. Model-agnostic permutation SHAP (Lundberg & Lee, 2017) will explain dropout probability using 25 training-only background rows and 67 evaluations. Signed member attributions will be summed within each semantic group, preserving additivity, before groups are ranked by absolute total. Deletion will mask those same groups with training references, recompute derived variables, and compare the cumulative absolute probability change with 50 random group orders; the primary statistic will be ranked-minus-random area with case-level uncertainty. Similar curves will indicate inadequate fidelity, and masking will measure reliance, not intervention. Validation checks reconstructed TabICL probabilities within 4.1e-8 and recovered known white-box attribution ranks with Spearman 1.0. Grouped permutation importance will supply global reliance, while ALE will examine three validation-selected numerical effects with less off-manifold risk than PDP under correlation.",
)

replace_text(
    find_one(document, "Following the rubric, we will pre-select 20 held-out cases"),
    "The stability evaluation will deterministically select four cases from each of five predicted-risk strata (20 total) without labels. For each case, the explainer will run 10 times, varying seed and training-background sample separately. Mean pairwise top-five overlap—shared features divided by five—will be primary, with rank correlation secondary. Continuous perturbations will draw Gaussian noise with standard deviation equal to 1% of the feature's training standard deviation, clip to training bounds, recompute dependencies, and qualify only when absolute probability change is at most 0.01. Mean overlap below the validation-frozen 0.80 warning level will be displayed, not hidden through selective reruns (Tiukhova et al., 2024).",
)
...
replace_text(
    ...
    "Before modelling, features will be classified by temporal availability and feasibility. Age, nationality, prior qualifications and completed first-semester outcomes will remain fixed. Potentially resolvable tuition or debt conditions may change only when realistic at the decision point, distinguishing institutional support from student-controlled action. For a flagged case, the search will find the smallest feasible change that moves it outside the cohort's top-20% set after recomputing its rank while all other scores stay fixed; no universal threshold will be assumed, and no-recourse cases will be explicit. These outputs will describe model behaviour, not causal promises (Karimi et al., 2021).",
)
...
replace_text(
    ...
    "After the prediction task, each participant will explain the judgement, propose a support action, and flag missing information. A pre-defined coding guide will group responses into comprehension, actionability and information-gap categories; two reviewers will independently code at least 20% of responses to report raw agreement, then resolve disagreements and report aggregate counts with paraphrased examples. This qualitative analysis will complement the forward-simulation results but will not train, validate or explain the dropout model.",
)
...
caption = document.add_paragraph(
    "Figure 1. Proposed research workflow. Model, capacity policy and explanation protocols will be frozen on validation data before one-time final testing. Ethics, governance and reproducibility will apply throughout."
)
...
replace_text(
    ...
    "Protected attributes—gender, nationality, age, international status, displaced status and special needs—will be treated as explicit audit attributes. Every group count will be retained; performance metrics will be suppressed rather than silently omitted below 30 records, and unavailable comparisons will be stated. Eligible groups will receive flag rate, true- and false-positive rates, precision, mean risk and calibration gap under the exact capacity labels. All non-protected model features will be screened as potential proxies using Cramér's V for categorical features and correlation-ratio eta for continuous features. These diagnostics will be descriptive, not proof of fairness.",
)

replace_text(
    find_one(document, "We will also examine plausible proxies"),
    "Plausible proxies will also be examined: parental occupation and qualification may encode socio-economic position, course and application mode may reflect demographic sorting, and debtor, tuition-fee and scholarship variables may encode financial disadvantage. Removing a protected attribute does not eliminate correlated proxy information, so influential proxies and limitations will be documented even where they weaken the model's apparent suitability. The system will support outreach prioritisation, not autonomous denial of education or services.",
)
...
replace_text(
    ...
    "The proposed contribution will extend beyond accuracy by asking whether TabICL's post-hoc local explanations merit trust and whether an intrinsically interpretable EBM offers a more defensible alternative. Same-test-set baselines, deletion faithfulness, white-box recovery, repeated-run and perturbation stability, human forward simulation, qualitative evidence and feasibility-constrained counterfactuals will form a coherent evaluation. Expected limitations include the single-institution UCI context, exclusion of unresolved Enrolled outcomes, approximate model-agnostic attribution, non-causal observational data, a small convenience sample, and likely non-professional audience proxies; therefore, the AIT study will test comprehensibility, not predictive validity or deployment suitability at AIT. If recruitment reaches only the minimum sample, the study will remain descriptive. Findings will remain valuable if EBM matches or exceeds TabICL, explanations are unstable or unfaithful, participants gain no benefit, or feasible recourse is unavailable.",
)
